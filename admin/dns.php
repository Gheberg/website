...
<body>
	...
	<div id="container">
				...
				<div class="inner">
					...
					<ul class="dash">
						...
						<span class="bubble"><?php echo $hotline; ?></span>
							<a href="dns.php?action=hotline&hotline=<?php echo $hotline; ?>">
		...
		<hr />
					<h1>Configurations DNS en attente</h1>
...
$date = date('d/m/Y');
$heure = date('H:i:s');
mysql_query("insert into log values ('', '$pseudo', '$date $heure', 'administrateur', 'Validation : DNS', 'administrateur')") or die(mysql_error());
mysql_query("DELETE FROM att_dns WHERE pseudo='$pseudo'");
}

$reponse = mysql_query("SELECT * FROM att_dns ORDER BY ID");
$i = 0;
while ($donnees = mysql_fetch_array($reponse) )
{
	$pseudo = nl2br(stripslashes($donnees['pseudo']));
	echo "$pseudo <form method=\"post\" action=\"dns.php?script=1&pseudo=$pseudo\"><input type=\"submit\" value=\"Accepter\"></form><br>";
	$i++;
}
if ($i == 0) echo "Aucune configuration en attente";
?>
					</table>
