...
{
	$search = $_POST['search'];
	$bon=mysql_query("select * from heberg_membres WHERE email='$search' OR tel='$search' OR nom='$search' OR prenom='$search'");
}
...
<form action="search_client.php?demande=1" method="post">
<table><tr><td>Adresse mail : </td><td><input type="text" size="25" name="search" value=""/></td><td><input type="submit" value="Envoyer" /></td></tr></table></form>

<?php
if ($_GET['demande'] == 1)
{
	if(mysql_num_rows($bon)==0) echo "Aucune information retrouvée";
	else
	{
		while($donnees = mysql_fetch_array($bon))
		{
